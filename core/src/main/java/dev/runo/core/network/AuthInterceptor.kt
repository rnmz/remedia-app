...
import dev.runo.core.common.Constants
...
class AuthInterceptor @Inject constructor(
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        return when {
            ...
            requiresApiKey != null -> handleRequiredApiKey(request, apiKey, method, endpoint, chain)
                ...
                Log.d(Constants.LOG_TAG, "[$method] $endpoint does not requiring an API key.")
    ...
    private fun handleOptionalApiKey(
        ...
        return if (apiKey.isNotBlank()) {
                Log.d(Constants.LOG_TAG, "[$method] $endpoint was executed successfully with an API key.")
                ...
                Log.d(Constants.LOG_TAG, "[$method] $endpoint was executed successfully without an API key.")
                chain.proceed(request)
            }
    }
    ...
    private fun handleRequiredApiKey(
        ...
        return if (apiKey.isNotBlank()) {
            Log.d(Constants.LOG_TAG, "[$method] $endpoint required an API key and was executed successfully.")
            val newRequest = request.newBuilder()
                .addHeader("Authorization", "Bearer $apiKey")
                .build()
            chain.proceed(newRequest)
            ...
            Log.w(Constants.LOG_TAG, "[$method] $endpoint not executed: API key is required but was null or empty.")
            Response.Builder()
